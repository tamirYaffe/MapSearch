...
public class MapGenerator {
    public int[][] generate(int row, int column) {
        ...
        maze[startPosition.getY()][startPosition.getX()] = 0;
    ...
    private boolean isEdgeWall(int[][] maze, Position randomWall) {
        Position top = new Position(randomWall.getY() - 1, randomWall.getX());
        Position bottom = new Position(randomWall.getY() + 1, randomWall.getX());
        Position left = new Position(randomWall.getY(), randomWall.getX() - 1);
        Position right = new Position(randomWall.getY(), randomWall.getX() + 1);
    ...
    private void connectEdgeWall(int[][] maze, ArrayList<Position> walls, Position randomWall) {
        ...
        if (!positionOnMaze(maze, top) || !positionOnMaze(maze, bottom)) {
            if (!checkTopBottomEdge(maze, randomWall))
                checkLeftRightEdge(maze, randomWall);
        } else {
            if (!checkLeftRightEdge(maze, randomWall))
                checkTopBottomEdge(maze, randomWall);
        }
        if (maze[randomWall.getY()][randomWall.getX()] == 1 && walls.size() < 2) {
            if (positionOnMaze(maze, left) && maze[left.getY()][left.getX()] == 0) {
                maze[randomWall.getY()][randomWall.getX()] = 0;
                maze[right.getY()][right.getX()] = 0;
                addWalls(maze, walls, right);
            }
            if (positionOnMaze(maze, right) && maze[right.getY()][right.getX()] == 0) {
                maze[randomWall.getY()][randomWall.getX()] = 0;
                maze[left.getY()][left.getX()] = 0;
                addWalls(maze, walls, left);
            }
            if (positionOnMaze(maze, top) && maze[top.getY()][top.getX()] == 0) {
                maze[randomWall.getY()][randomWall.getX()] = 0;
                maze[bottom.getY()][bottom.getX()] = 0;
                addWalls(maze, walls, bottom);
            }
            if (positionOnMaze(maze, bottom) && maze[bottom.getY()][bottom.getX()] == 0) {
                maze[randomWall.getY()][randomWall.getX()] = 0;
                maze[top.getY()][top.getX()] = 0;
    ...
    private boolean checkTopBottomRegular(int[][] maze, ArrayList<Position> walls, Position randomWall) {
        ...
        if (maze[top.getY()][top.getX()] == 0 && maze[bottom.getY()][bottom.getX()] == 1) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            maze[bottom.getY()][bottom.getX()] = 0;
            addWalls(maze, walls, bottom);
            return true;
        }
        if (maze[top.getY()][top.getX()] == 1 && maze[bottom.getY()][bottom.getX()] == 0) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            maze[top.getY()][top.getX()] = 0;
            addWalls(maze, walls, top);
            return true;
    ...
    private boolean checkLeftRightRegular(int[][] maze, ArrayList<Position> walls, Position randomWall) {
        ...
        if (maze[left.getY()][left.getX()] == 0 && maze[right.getY()][right.getX()] == 1) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            maze[right.getY()][right.getX()] = 0;
            addWalls(maze, walls, right);
            return true;
        }
        if (maze[left.getY()][left.getX()] == 1 && maze[right.getY()][right.getX()] == 0) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            maze[left.getY()][left.getX()] = 0;
            addWalls(maze, walls, left);
            return true;
     ...
     * @return true if we the input random wall is separating between two passes(top and bottom) in the maze path, otherwise false.
     */
    private boolean checkTopBottomEdge(int[][] maze, Position randomWall) {
        Position top = new Position(randomWall.getY() - 1, randomWall.getX());
        Position bottom = new Position(randomWall.getY() + 1, randomWall.getX());
        if ((positionOnMaze(maze, top) && maze[top.getY()][top.getX()] == 0) && !positionOnMaze(maze, bottom)) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            return true;
        }
        if ((positionOnMaze(maze, bottom) && maze[bottom.getY()][bottom.getX()] == 0) && !positionOnMaze(maze, top)) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            return true;
        }
        return false;
     ...
     * @return true if we the input random wall is separating between two passes(left and right) in the maze path, otherwise false.
     */
    private boolean checkLeftRightEdge(int[][] maze, Position randomWall) {
        Position left = new Position(randomWall.getY(), randomWall.getX() - 1);
        Position right = new Position(randomWall.getY(), randomWall.getX() + 1);
        if ((positionOnMaze(maze, left) && maze[left.getY()][left.getX()] == 0) && !positionOnMaze(maze, right)) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            return true;
        }
        if ((positionOnMaze(maze, right) && maze[right.getY()][right.getX()] == 0) && !positionOnMaze(maze, left)) {
            maze[randomWall.getY()][randomWall.getX()] = 0;
            return true;
        }
        return false;
    ...
    private void addWalls(int[][] maze, ArrayList<Position> walls, Position pos) {
        for (int i = -1; i <= 1; i++)
            for (int j = -1; j <= 1; j++)
                if (i != j && !(i == 1 && j == -1) && !(i == -1 && j == 1) && positionOnMaze(maze, pos.getY() + i, pos.getX() + j))
                    if (maze[pos.getY() + i][pos.getX() + j] == 1)//cell is a wall
                        walls.add(new Position(pos.getY() + i, pos.getX() + j));
    }
    ...
    private boolean positionOnMaze(int[][] maze, Position pos) {
        return positionOnMaze(maze, pos.getY(), pos.getX());
    }
...
//        }while (maze[endRow][endColumn]==1 || (endRow==startPosition.getY() && endColumn==startPosition.getX()));
